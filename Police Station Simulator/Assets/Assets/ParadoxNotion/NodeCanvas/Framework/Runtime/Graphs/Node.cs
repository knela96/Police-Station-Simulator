...
namespace NodeCanvas.Framework{
	...
	abstract public partial class Node : IGraphElement{
		...
		protected Status Error(string msg){
            ParadoxNotion.Services.Logger.LogError(string.Format("{0} | On Node '{1}' | ID '{2}' | Graph '{3}'", msg, name, ID, graph.name), "Execution", this);
			status = Status.Error;
			return Status.Error;
		}

		///A little helper function to log errors easier
		protected Status Error(System.Exception e){
