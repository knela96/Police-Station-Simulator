			...
			if(col.gameObject != gameObject && GeometryUtility.TestPlanesAABB(planes, col.bounds))
			{
            	...
            	if(Physics.Raycast(ray, out hit, frustum.farClipPlane, ray_mask))
            	{
                    if (hit.collider.gameObject.CompareTag("DetectEntity"))
                    {
                        ...
                        {
                            if (col.gameObject.GetComponent<CriminalBehavior>().escape && !gameObject.GetComponent<PoliceBehaviour>().to_cell && !gameObject.GetComponent<PoliceBehaviour>().receptionist && !gameObject.GetComponent<PoliceBehaviour>().detected)
                            {
